...
> Live demo: **<https://duetstudio.mehmetcan.me>** · Demo video: **<https://youtu.be/0sRPp8rAA3c>**
...
- **Chrome DevTools**: open **Application → WebMCP** on the page. It lists every registered tool with its title, description, schema and annotations, logs each invocation with its input and output, and lets you run a tool by hand. Select a range in the grid and watch `edit_selection` appear in the list.
  ...
  ![Chrome DevTools, Application → WebMCP: the Tool Activity table with seven calls, their inputs and outputs (including the actionable error from an unknown track), and the Available Tools list where edit_selection is registered because the human is holding a selection](docs/devtools-webmcp.png)
