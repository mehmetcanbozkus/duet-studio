...
- `bun run evals` runs Chrome's own `webmcp-evals` harness in `smoke` mode over `evals/duet-studio.json`: 14 cases of "what a user would say" plus the tool calls each should produce, all 21 steps executed against the live page with no LLM and no API key. `scripts/webmcp-evals.mjs` calls the evaluator directly and injects our Chromium launcher, because the published CLI only launches a real Google Chrome install; that reaches past the package's public surface, so `webmcp-evals` is pinned to an exact version.
...
- `bun run drive` keeps one WebMCP-capable Chromium alive between invocations so the tools can be called by hand, the way an agent works a session: `start [url] [--headless] [--fresh]`, `list`, `schema <tool>`, `call <tool> '<json>'`, `demo` (a whole scripted session), `click`, `shot`, `eval`, `logs`, `stop`. Use it to reproduce agent behaviour and to regenerate `docs/screenshot.png`.
- They all default to http://localhost:3000/ and Chromium at /usr/bin/chromium; override with `SMOKE_URL=http://localhost:3123/` and `CHROMIUM=/path/to/chrome`. The app runs without any environment variables; the two it looks at (`VERCEL` at build time, `BLOB_READ_WRITE_TOKEN` at request time) are optional and documented in `.env.example` and Stack.
